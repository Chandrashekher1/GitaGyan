...
function getLandmark(landmarks: PoseLandmark[], index: number): PoseLandmark {
  return landmarks[index]!;
}

function getAngle(a: PoseLandmark, b: PoseLandmark, c: PoseLandmark): number {
  const radians =
...
function penaltyForRange(
  ...
  return clamp((diff / Math.max(failDelta, 0.0001)) * maxPenalty, 0, maxPenalty);
}
...
function hasReliableLandmarks(landmarks: PoseLandmark[]): boolean {
  ...
  const visibleLandmarks = landmarks.filter(
    (landmark) =>
      ...
      (landmark.visibility === undefined || landmark.visibility > 0.15)
  );

  return visibleLandmarks.length >= 24;
}

function inferNextPose(poseName: string): string {
  const poseIndex = POSE_SEQUENCE.indexOf(poseName);
  if (poseIndex === -1) {
    return poseName;
  }

  return POSE_SEQUENCE[(poseIndex + 1) % POSE_SEQUENCE.length]!;
}

export function analyzeYogaPose(
  landmarks: PoseLandmark[],
  poseName: string
): PoseAnalysis {
  if (!hasReliableLandmarks(landmarks)) {
    return {
      score: 0,
  ...
  const addFinding = (penalty: number, issue: string, correction: string) => {
    ...
    findings.push({ penalty, issue, correction });
  };
  ...
  if (poseName.includes('Tadasana') || poseName.includes('Mountain')) {
    ...
    const hipFold = average([
      getAngle(leftShoulder, leftHip, leftAnkle),
      getAngle(rightShoulder, rightHip, rightAnkle),
    ]);
    addFinding(
    ...
    const foldAngle = average([
      getAngle(leftShoulder, leftHip, leftKnee),
      getAngle(rightShoulder, rightHip, rightKnee),
    ]);
    addFinding(
