...
export async function getSemanticChatHistory(
  userMessage: string,
  sessionId: string,
  topK: number = 4
) {
  try {
    ...
    const results: { role: string; content: string; emotion: string; timestamp: string }[] = [];
...
export async function getRecentChatHistory(sessionId: string, limit: number = 6) {
  try {
    const db = connectToDatabase();
    const collection = db.collection(CHAT_HISTORY_COLLECTION);
    ...
    const docs: { role: string; content: string; timestamp: string }[] = [];
    for await (const doc of cursor) {
      docs.push({
        role: doc.role,
        content: doc.content,
...
export async function tripleRAGQuery(
  userMessage: string,
  emotionLabel: string,
  sessionId: string
) {
  ...
  const [gitaResults, mentalResults, semanticHistory, recentHistory] = await Promise.all([
    ...
    (async () => {
      ...
      const cursor = gitaCollection.find(
        {},
        { sort: { $vectorize: userMessage }, limit: k_gita, projection: { text: true } }
      );
      for await (const doc of cursor) {
    ...
    (async () => {
      const results: string[] = [];
      try {
        const cursor = mentalCollection.find(
          {},
          { sort: { $vectorize: userMessage }, limit: k_mental, projection: { content: true } }
        );
        for await (const doc of cursor) {
